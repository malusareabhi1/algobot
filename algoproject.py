...
""")
    ...
    api_secret = st.text_input("Enter your API Secret", type="password")
     ...
     # If login already done → skip login block and show tabs immediately
    if st.session_state.kite:
        st.success("Already logged in to Zerodha 🎉")
        kite = st.session_state.kite
    else:
...
#######################################################zerodha place order####################################################################################
    ...
    #######################################################################################
        ...
        if signal:
            ...
            option_data["tradingsymbol"] = build_zerodha_tradingsymbol(
                underlying=option_data.get("underlying", "NIFTY"),
                expiryDate=option_data["expiryDate"],
                strikePrice=option_data["strikePrice"],
                option_type="CE" if "CE" in option_data["identifier"] else "PE"
            )
