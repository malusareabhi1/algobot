...
#-------------------------------------
    ...
    def get_live_1minute_data(kite, symbol="NIFTY 50", interval="minute", days=2):
        ...
        try:
            ...
            st.error(f"Error fetching 1-minute data: {e}")
            return pd.DataFrame()

    # -------------------------
    # 2. MAKE 15-MIN CANDLES
    # -------------------------
    ...
    # -------------------------
    # 3. GET LIVE DATA SAFELY
    # -------------------------
    if "kite" not in st.session_state or not st.session_state.kite:
        st.warning("Please login to Kite first.")
        st.stop()

    kite = st.session_state.kite

    df_1min_live = get_live_1minute_data(kite)

    if df_1min_live.empty:
        st.warning("No live 1-minute data received.")
        st.stop()
    ...
    if df_15.empty:
        st.warning("15-minute candle not formed yet.")
        st.stop()

    # -------------------------
    # 5. CHECK NEW CANDLE CLOSE
    # -------------------------
    ...
    if last15 != st.session_state.last_checked:
        ...
        if signal:
            ...
            st.info("15m candle closed - No signal.")
