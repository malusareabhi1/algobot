...
with st.sidebar:
    ...
    MENU = st.radio(
        "Navigate",
        ["Home", "Strategies","My Account", "Zerodha Broker API","Groww Broker API", "Dashboard","Backtest","Live Trade","Paper Trade", "Products", "Support","Live Trade2","Test1","Logout"],
        index=0,
    )
...
# ------------------------------------------------------------
   ...
   #------------------------------------------------------------------------------------------------------------------------------------
    ...
    def show_fund_status(kite, lot_size, premium):
        try:
            funds = kite.margins(segment="equity")   # or "commodity" if needed
    
            available_cash = funds["available"].get("cash", 0)
            utilised_margin = funds.get("utilised", {}).get("debits", 0)
    
            total_balance = available_cash - utilised_margin
    
            st.subheader("💰 Zerodha Fund Status")
    
            col1, col2, col3 = st.columns(3)
            col1.metric("Available Cash", f"₹{available_cash:,.2f}")
            col2.metric("Used Margin", f"₹{utilised_margin:,.2f}")
...
#--------------------------------------------------------------------------------------------------------
    ...
    # ---- imports & aliasing to avoid conflicts ----
    from streamlit_autorefresh import st_autorefresh
    import time as pytime                 # avoid conflict with datetime.time
    ...
    # ---- session-state init ----
    if "order_executed_map" not in st.session_state:
        # map candle_timestamp_iso -> True (prevents duplicate orders for same candle)
        st.session_state.order_executed_map = {}

    if "order_logs" not in st.session_state:
        st.session_state.order_logs = []  # list of dicts with order info

    if "kite" not in st.session_state:
        st.warning("⚠ Please connect Kite first from sidebar and reload this page.")
        st.stop()

    kite = st.session_state.kite  # KiteClient instance

    # ---- page config (safe to call multiple times) ----
    st.set_page_config(layout="wide")

    # ---- market hours auto-refresh (1 minute when market open) ----
    ist = pytz.timezone("Asia/Kolkata")
    ...
    if market_start <= now_ist <= market_end:
        # refresh every 60 seconds while market is open
        st_autorefresh(interval=60 * 1000, key="live_trade2_refresh")
    else:
        st.info("Auto-refresh paused — Outside market hours (09:30–15:25 IST)")

    st.header("Nifty 15-min Chart (Live)")

    # ---- user inputs for live order placement ----
    st.sidebar.header("Live Order Settings")
    tradingsymbol_for_entry = st.sidebar.text_input(
        "Tradingsymbol to place order (e.g. NFO:NIFTY24DEC22500CE or NFO:NIFTY24DECSELL)", ""
    )
    entry_qty = st.sidebar.number_input("Order Quantity", min_value=1, value=1, step=1)
    entry_side_for_bull = st.sidebar.selectbox("Bullish order side", ["BUY", "SELL"], index=0)
    entry_side_for_bear = st.sidebar.selectbox("Bearish order side", ["SELL", "BUY"], index=0)
    place_market_orders = st.sidebar.checkbox("Place Market Orders automatically", value=False)
    place_sl_after_entry = st.sidebar.checkbox("Place SL order after entry (SLM/SL)", value=False)
    sl_trigger_offset = st.sidebar.number_input("SL trigger offset (points)", min_value=1, value=50, step=1)

    # ---- date selection & fetch 15-min data for last ~8 days ----
    selected_date = st.date_input("Select date (used for plotting/data window)", value=datetime.today())
    start_date = selected_date - timedelta(days=8)
    end_date = selected_date + timedelta(days=1)

    df = yf.download("^NSEI", start=start_date.strftime("%Y-%m-%d"),
                     end=end_date.strftime("%Y-%m-%d"), interval="15m", progress=False)

    if df.empty:
        st.warning("No data downloaded for the selected range.")
        st.stop()

    # normalize downloaded columns (yfinance uses Datetime index)
    df = df.reset_index()
    # handle possible column name variations - prefer 'Datetime' as your code expected earlier
    if 'Datetime' in df.columns:
        ...
        # yfinance uses 'Datetime' as index; convert index
        df.rename(columns={df.columns[0]: 'Datetime'}, inplace=True)
        datacol = 'Datetime'

    # timezone handling: make tz-aware in Asia/Kolkata
    ...
    col_high = find_col_like(cols, 'high')
    col_low = find_col_like(cols, 'low')
    col_close = find_col_like(cols, 'close')

    # rename to uniform names
    df.rename(columns={col_open: 'Open', col_high: 'High', col_low: 'Low', col_close: 'Close'}, inplace=True)

    # filter out data for last two trading days (like you did)
    unique_days = pd.unique(df['Datetime'].dt.date)
    if len(unique_days) < 2:
        st.warning("Not enough data for two trading days.")
        # still show chart and stop further live checks
    ...
    last_day = unique_days[-2]
    today_day = unique_days[-1]
    df_plot = df[df['Datetime'].dt.date.isin([last_day, today_day])].copy()

    # get last_day 3:00 PM candle - base zone
    candle_3pm = df_plot[(df_plot['Datetime'].dt.date == last_day) &
                         (df_plot['Datetime'].dt.hour == 15) &
                         (df_plot['Datetime'].dt.minute == 0)]
    ...
    if base_low is not None and base_high is not None:
        fig.add_hline(y=base_low, line_dash="dash", annotation_text="Base Low", annotation_position="bottom left")
        fig.add_hline(y=base_high, line_dash="dash", annotation_text="Base High", annotation_position="top left")
    fig.update_layout(title="Nifty 15-min candles - Last Day & Today", xaxis_rangeslider_visible=False)
    st.plotly_chart(fig, use_container_width=True)

    # --------------------------------------------
    # SIGNAL DETECTION for the latest candle
    # --------------------------------------------
    df_signal = df_plot[['Datetime', 'Open', 'High', 'Low', 'Close']].copy()
    df_signal['Signal'] = "No Signal"
    df_signal['Reason'] = "-"

    # We'll evaluate the latest candle of today (most recent row whose date == today_day)
    df_today = df_signal[df_signal['Datetime'].dt.date == today_day]
    if df_today.empty:
        st.info("No today's candles available yet.")
        st.stop()

    latest_candle = df_today.iloc[-1]            # last candle row
    latest_ts = latest_candle['Datetime']        # timezone-aware Timestamp
    latest_ts_iso = latest_ts.isoformat()        # key for session-state map

    # find the 9:30 candle of today (if present) - your previous logic used only 9:30 candle to decide signals
    nine_thirty_candle = df_today[df_today['Datetime'].dt.strftime("%H:%M") == "09:30"]
    # but we will evaluate the latest candle each time for signal as requested
    C = latest_candle
    C_high = C['High']
    C_low = C['Low']
    C_close = C['Close']
    ...
    # ensure base zone exists
    if base_low is not None and base_high is not None:
        # Preserve your same conditions but evaluate on the latest candle
        if (C_low < base_high) and (C_close > base_high):
            signal = "Bullish"
            reason = "Condition 1: Break Above Base Zone"
        elif C_close < base_low:
            signal = "Bearish"
            reason = "Condition 2: Major Gap Down"
        elif C_close > base_high:
            signal = "Bullish"
            reason = "Condition 3: Major Gap Up"
        elif (C_high > base_low) and (C_close < base_low):
            signal = "Bearish"
            reason = "Condition 4: Break Below Base Zone"

    # show detected signal in UI
    st.subheader("Latest Candle Signal")
    st.write("Latest candle time:", latest_ts)
    st.write("Detected Signal:", signal if signal else "No Signal")
    if reason:
    ...
    # Place order only if:
    #  - signal exists
    #  - we haven't already executed for this candle timestamp
    #  - user allowed automatic market orders
    # --------------------------------------------
    executed_before = st.session_state.order_executed_map.get(latest_ts_iso, False)

    if signal and (not executed_before):
        st.info("Signal found for the latest candle and not executed before.")
        # Show an on-screen "Place Order" button for manual control (safer)
        col1, col2 = st.columns([1, 3])
        with col1:
            place_now = st.button("Place Order Now")
        with col2:
            st.write("Auto-place orders checkbox (sidebar):", place_market_orders)

        if place_now or place_market_orders:
            if not tradingsymbol_for_entry:
                st.error("Please enter Tradingsymbol in sidebar to place order.")
            else:
                side = entry_side_for_bull if signal == "Bullish" else entry_side_for_bear
                qty = int(entry_qty)

                # Try placing market order using your KiteClient wrapper
                try:
                    order_response = kite.place_order_market(tradingsymbol_for_entry, qty, side=side)
                except Exception as e:
                    order_response = {"error": str(e)}

                # Save log and mark executed
                log = {
                    "timestamp": latest_ts_iso,
                    "signal": signal,
                    "reason": reason,
                    "tradingsymbol": tradingsymbol_for_entry,
                    "side": side,
                    "qty": qty,
                    "order_response": order_response,
                    "when": datetime.now(ist).isoformat()
                }
                st.session_state.order_logs.append(log)
                st.session_state.order_executed_map[latest_ts_iso] = True
                st.success(f"Order placed (or attempted). See order logs below.")
                st.json(order_response)

                # Optionally place SL order after entry
                if place_sl_after_entry and isinstance(order_response, (dict,)) and order_response.get('error') is None:
                    # compute SL trigger price simplistic example: if BUY then trigger = filled_price - offset
                    # NOTE: Kite place_order returns an order_id and not immediate fill data; adjust in production.
                    try:
                        if side == "BUY":
                            trigger_price = max(0.0, C_close - float(sl_trigger_offset))
                            sl_side = "SELL"
                        else:
                            trigger_price = max(0.0, C_close + float(sl_trigger_offset))
                            sl_side = "BUY"

                        sl_resp = kite.place_order_sl(tradingsymbol_for_entry, qty, sl_side, trigger=trigger_price, price=None)
                        st.session_state.order_logs.append({
                            "timestamp": latest_ts_iso,
                            "action": "SL_PLACE",
                        ...
                        st.success("SL order placement attempted.")
                    except Exception as e:
                        st.error(f"SL placement error: {e}")

    elif signal and executed_before:
        st.info("Signal present, but orders already executed for this candle (skipping duplicate).")

    else:
        st.info("No actionable signal on the latest candle.")

    # --------------------------------------------
    # Display recent signal/order logs
    # --------------------------------------------
    st.subheader("Order Logs (recent)")
    if st.session_state.order_logs:
        df_logs = pd.DataFrame(st.session_state.order_logs)
        st.dataframe(df_logs.sort_values("when", ascending=False).reset_index(drop=True), use_container_width=True)
    else:
        st.write("No orders placed in this session yet.")

    # --------------------------------------------
    # Also show the filtered signal table (only actual signals)
    # --------------------------------------------
    df_signal['Signal'] = df_signal.apply(
        lambda r: ("Bullish" if (r['Low'] < base_high and r['Close'] > base_high) else
                   ("Bearish" if r['Close'] < base_low else
                    ("Bullish" if r['Close'] > base_high else
                     ("Bearish" if (r['High'] > base_low and r['Close'] < base_low) else "No Signal")))),
        axis=1
    )
    df_show = df_signal[df_signal['Signal'] != "No Signal"].copy()
    df_show.rename(columns={'Datetime': 'Time'}, inplace=True)
    df_show = df_show[['Time', 'Open', 'High', 'Low', 'Close', 'Signal']]
    st.subheader("All Signal Candles (last 2 days)")
    st.dataframe(df_show, use_container_width=True)
